...
export default function Komedi() {
  ...
  const [komediArtikel, setKomediArtikel] = useState([]);

  useEffect(() => {
    loadKomediArtikel();
  }, []);

  const loadKomediArtikel = async () => {
  try {
    const artikel = await store.artikel.getArtikel();
    const komediArtikelData = artikel.body.data.filter((artikel) => {
      if (Array.isArray(artikel.kategoriArtikel)) {
        return artikel.kategoriArtikel.includes('komedi');
      } else {
        return artikel.kategoriArtikel === 'komedi';
      }
    });
    setKomediArtikel(komediArtikelData);
    console.log(komediArtikelData);
  } catch (error) {
  ...
  return (
    ...
    <h1 className="text-4xl pb-4">Komedi Cerpen</h1>
      <div className="bg-gray-50 p-8">
      
      {komediArtikel.map((artikel) => (
