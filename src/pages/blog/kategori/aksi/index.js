...
export default function Aksi() {
  ...
  const [aksiArtikel, setAksiArtikel] = useState([]);

  useEffect(() => {
    loadAksiArtikel();
  }, []);

  const loadAksiArtikel = async () => {
  try {
    const artikel = await store.artikel.getArtikel();
    const aksiArtikelData = artikel.body.data.filter((artikel) => {
      if (Array.isArray(artikel.kategoriArtikel)) {
        return artikel.kategoriArtikel.includes('aksi');
      } else {
        return artikel.kategoriArtikel === 'aksi';
      }
    });
    setAksiArtikel(aksiArtikelData);
    console.log(aksiArtikelData);
  } catch (error) {
  ...
  return (
    ...
    <h1 className="text-4xl pb-4">Aksi Cerpen</h1>
      <div className="bg-gray-50 p-8">
      
      {aksiArtikel.map((artikel) => (
